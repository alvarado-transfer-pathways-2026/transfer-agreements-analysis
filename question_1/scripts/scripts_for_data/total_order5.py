...
def count_required_courses(df, selected_schools, articulated_tracker, unarticulated_tracker):
    ...
    for (uc, group_id), group_df in filtered_df.groupby(['UC Name', 'Group ID']):
        ...
        for set_id, set_df in group_df.groupby('Set ID'):
            ...
            if fallback_receiving is None or len(all_receiving_courses) < len(fallback_receiving or set()):
...
def process_combinations_order_sensitive(df, uc_list):
    ...
    uc_role_totals = {
        uc: {f"{i+1}th": {'articulated': 0, 'unarticulated': 0} for i in range(5)} for uc in uc_list
    }
    # Fix ordinal suffixes for 1st, 2nd, 3rd, 4th, 5th
    for uc in uc_list:
        uc_role_totals[uc]['1st'] = uc_role_totals[uc].pop('1th')
        uc_role_totals[uc]['2nd'] = uc_role_totals[uc].pop('2th')
        uc_role_totals[uc]['3rd'] = uc_role_totals[uc].pop('3th')
        uc_role_totals[uc]['4th'] = uc_role_totals[uc].pop('4th')
        uc_role_totals[uc]['5th'] = uc_role_totals[uc].pop('5th')
...
def process_all_csvs(folder_path):
    ...
    uc_roles = ['1st', '2nd', '3rd', '4th', '5th']
    
    overall_totals = {
        uc: {role: {'articulated': 0, 'unarticulated': 0} for role in uc_roles} for uc in uc_schools
    }
    ...
    for idx, file in enumerate(csv_files):
        ...
        # 9P5 = 15120 permutations, each UC appears in each position 8P4 = 1680 times
        per_uc_per_position = 1680  # 8P4 = 1680
        ...
        avg = {
            ...
            } for role in uc_roles} for uc in uc_schools
        }
    ...
    for idx, role in enumerate(uc_roles):
        data = []
        filtered_pairs = []
        filtered_sum = {}
        filtered_count = {}
